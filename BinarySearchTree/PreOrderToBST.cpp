...
Node* constructBST_util(int pre[], int minVal, int maxVal, int& currIdx, int arrSize) {
	if(currIdx >= arrSize || minVal > maxVal)
		return NULL;
	
	if(pre[currIdx] < minVal || pre[currIdx] > maxVal)
		return NULL;
	
	Node *root = newNode(pre[currIdx]);
	currIdx += 1;
	root->left = constructBST_util(pre, minVal, root->data-1, currIdx, arrSize);
	root->right = constructBST_util(pre, root->data+1, maxVal, currIdx, arrSize);
	return root;
}
...
{
	int currIdx = 0;
	Node *root = constructBST_util(pre, INT_MIN, INT_MAX, currIdx, size);
	return root;
}
