...
void convert(Node *head, TreeNode *&root) {
    ...
    // The linked list here is the BFS of tree
    queue<TreeNode*> treeLevelNodes;

    TreeNode* tree_root = new TreeNode(head->data);

    treeLevelNodes.push(tree_root);
    head = head->next;
    
    while(head != NULL) {
        TreeNode *curr = treeLevelNodes.front();
        treeLevelNodes.pop();
