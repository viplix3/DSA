...
bool BFS(vector<int> adjacencyList[], int sourceVertex, bool visited[]) {
	queue<int> gnodes;
	...
	while(gnodes.empty() == false) {
		...
		for(auto connected_node : adjacencyList[curr]) {
			cycle = cycle & visited[connected_node];
		...
		if(cycle && adjacencyList[curr].size() > 1)
			return true;
	}
...
bool checkForCycle(vector<int> adjacencyList[], int numVertices) {
	...
	for(int i=0; i<numVertices; i++)
		if(visited[i] == false)
			if(BFS(adjacencyList, i, visited) == true)
				return true;
