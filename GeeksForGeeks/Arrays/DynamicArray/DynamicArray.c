...
void setItem(DynamicArray *array_object, int index, int item){
	if( (index < ZERO_FLAG) || (size(array_object) < index)){
		printf("Provided index %d is invalid. It's beyond current array size %d.\n", 
			index, size(array_object));
		printf("As array is not filled at provided position, element won't be inserted.\n");
		return;
	}
